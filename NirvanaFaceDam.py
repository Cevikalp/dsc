...
from Networks.model_irse import IR_101
...
def main(args):
    save_dir = os.path.join('logs','NirvanaFaceLRelu','%s_%s_lr%.6f_Nirvana_Expand%d_Epoch%d_Seed%d_div%d'%(args.dataset_name,args.Network,args.lr, args.Expand, args.epochs, args.Seed, args.div))
    ...
    # lfw, lfw_issame = get_val_data('./data')
    # lfw, lfw_issame = get_val_data('./data')
    lfw, cfp_ff, cfp_fp, agedb_30, calfw, cplfw, lfw_issame, cfp_ff_issame, cfp_fp_issame, agedb_30_issame, calfw_issame, cplfw_issame = get_val_data('/run/media/mlcv/SSD2/Hasan/NirvanaFace/data')
    ...
    model = DAMGeneralNew(num_dim=20000, num_layers=1)
    model.to(device)
    # args.feat_dim = args.num_classes-1
    print(args)
...
def parse_args():
    ...
    parser.add_argument('--Expand', default=64, type=int, metavar='N',
